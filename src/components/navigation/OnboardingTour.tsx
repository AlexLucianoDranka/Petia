...
import { Sparkles, Calendar, Dog, CreditCard, CheckCircle2, ArrowRight, X, ShieldCheck } from 'lucide-react';
...
export function OnboardingTour() {
  ...
  useEffect(() => {
    // Check if new account or tour not completed
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="card rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-6 border border-st-electric/50 shadow-2xl animate-fade-up relative overflow-hidden bg-gradient-to-b from-st-navy via-st-surface to-st-surface">
        ...
        <button
          ...
          className="absolute top-4 right-4 p-2 text-st-muted hover:text-st-arctic rounded-xl hover:bg-st-surface transition-colors"
          title="Pular Tour"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="pt-4 border-t border-st-border/40 flex items-center justify-between gap-3">
          <button
            onClick={handleComplete}
            className="text-xs font-bold text-st-muted hover:text-st-arctic transition-colors px-2 py-1"
          >
            Pular Guia
          </button>
          ...
          <button
            onClick={handleNext}
            className="px-6 py-3 rounded-xl bg-st-electric hover:bg-st-steel text-white font-extrabold text-xs shadow-glow transition-all flex items-center gap-2 border-none whitespace-nowrap"
          >
